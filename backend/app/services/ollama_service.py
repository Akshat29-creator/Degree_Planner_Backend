...
"""
    ...
    async def generate_resume(self, inputs: dict) -> str:
        """
        Generate a professional resume from structured inputs.
        
        Args:
            inputs: Dict with name, education, skills, projects, experience
            
        Returns:
            Formatted resume text
        """
        prompt = f"""Generate a professional resume in plain text format.
...
2. Include all provided information
3. Use bullet points for lists
4. Keep it concise (1-2 pages worth)
5. Include relevant sections: Summary, Education, Skills, Projects, Experience

Generate the resume now:"""

        system = "You are a professional resume writer. Create clean, ATS-friendly resumes."
        
        try:
            response = await self._call_ollama(prompt, system)
            return response
        except Exception as e:
            print(f"[DEBUG] Resume generation error: {e}")
            # Fallback: create simple resume
            resume = f"""
{inputs.get('name', 'Candidate')}
{'=' * 50}

EDUCATION
{chr(10).join([f"• {e.get('degree', '')} - {e.get('institution', '')} ({e.get('year', '')})" for e in inputs.get('education', [])])}

SKILLS
{', '.join(inputs.get('skills', ['Not specified']))}

PROJECTS
{chr(10).join([f"• {p.get('title', '')}: {p.get('description', '')}" for p in inputs.get('projects', [])])}

EXPERIENCE
{chr(10).join([f"• {e.get('role', '')} at {e.get('company', '')} ({e.get('duration', '')})" for e in inputs.get('experience', [])])}
"""
            return resume.strip()
    ...
    async def generate_interview_questions(
        ...
        """
        Generate interview questions based on skill type and optional resume.
        
        Args:
            skill_type: Type of interview (Backend, DSA, ML, HR, etc.)
            question_count: Number of questions to generate
            resume_text: Optional resume for personalized questions
            
        Returns:
            List of question dicts with question, type, difficulty
        """
        resume_context = ""
        if resume_text:
            resume_context = f"""
...
Include 2-3 questions based on the candidate's resume (projects, experience, skills).
"""

        prompt = f"""Generate {question_count} interview questions for a {skill_type} interview.

{resume_context}

INTERVIEW STRUCTURE (follow this order):
1. Warmup question (introduce yourself, background)
2. Resume-based questions (if resume provided)
3. Technical questions ({skill_type} specific, progressively harder)
4. Scenario/Problem-solving questions
5. Behavioral/HR questions
6. Closing question (any questions for us?)

OUTPUT FORMAT (JSON array):
[
    {{"question": "...", "type": "warmup", "difficulty": "easy"}},
    {{"question": "...", "type": "technical", "difficulty": "medium"}},
    {{"question": "...", "type": "scenario", "difficulty": "hard"}},
    ...
]

Types: warmup, resume_based, technical, scenario, behavioral, closing
Difficulties: easy, medium, hard

Generate exactly {question_count} questions as a JSON array:"""

        system = f"You are an expert {skill_type} interviewer. Generate realistic, progressive interview questions."
        ...
        # Fallback questions
        fallback = [
            {"question": "Tell me about yourself and your background.", "type": "warmup", "difficulty": "easy"},
            {"question": f"What interests you about {skill_type}?", "type": "behavioral", "difficulty": "easy"},
            {"question": f"Explain a fundamental concept in {skill_type}.", "type": "technical", "difficulty": "medium"},
            {"question": f"How would you approach debugging a complex issue in {skill_type}?", "type": "technical", "difficulty": "medium"},
            {"question": "Describe a challenging project you worked on and how you overcame obstacles.", "type": "scenario", "difficulty": "medium"},
            {"question": f"What's a recent trend in {skill_type} that excites you?", "type": "technical", "difficulty": "easy"},
            {"question": "How do you handle disagreements with team members?", "type": "behavioral", "difficulty": "medium"},
            {"question": "Where do you see yourself in 5 years?", "type": "behavioral", "difficulty": "easy"},
            {"question": "What questions do you have for us?", "type": "closing", "difficulty": "easy"},
            {"question": f"Design a simple system using {skill_type} principles.", "type": "scenario", "difficulty": "hard"},
        ]
    ...
    async def analyze_interview_answer(
        ...
        """
        Analyze a user's interview answer.
        
        Args:
            question: The interview question
            answer: User's answer
            skill_type: Interview skill type for context
            
        Returns:
            Analysis dict with scores and feedback
        """
        prompt = f"""Analyze this interview answer for a {skill_type} position.

QUESTION: {question}

CANDIDATE'S ANSWER: {answer}

ANALYZE ON THESE CRITERIA (score 0-100 each):
1. Correctness - Is the answer technically/factually correct?
...
{{
    ...
    "completeness": 70,
    "relevance": 85,
    "feedback": "Brief constructive feedback for the candidate",
...
Provide your analysis as JSON:"""
        ...
        system = f"You are an expert {skill_type} interviewer evaluating candidate responses. Be fair but constructive."
        
        try:
            response = await self._call_ollama(prompt, system)
            analysis = self._extract_json(response)
            
            if isinstance(analysis, dict) and "correctness" in analysis:
                return analysis
        except Exception as e:
            print(f"[DEBUG] Answer analysis error: {e}")
        ...
        return {
            ...
            "completeness": 70,
            "relevance": 70,
            "feedback": "Answer recorded. Continue to the next question.",
    ...
    async def generate_interview_report(
        ...
        Args:
            skill_type: Type of interview
            questions: List of Q&A with scores and analyses
            
        Returns:
            Complete report with scores and insights
        """
        # Prepare summary of Q&A
        qa_summary = ""
        for i, q in enumerate(questions, 1):
            qa_summary += f"""
Q{i}: {q.get('question', '')[:100]}
A{i}: {q.get('answer', '')[:150]}
Score: {q.get('score', 0)}/100
"""

        prompt = f"""Generate a comprehensive interview performance report.

INTERVIEW TYPE: {skill_type}
QUESTIONS AND ANSWERS:
{qa_summary}

Generate a detailed report with:
1. Scores (0-100) for: technical_score, communication_score, confidence_score, problem_solving_score
2. strengths (3-5 bullet points)
3. weaknesses (2-3 bullet points)
4. improvements (3-5 actionable suggestions)
5. retry_recommendations (topics to practice)
6. confidence_insights (observation about confidence level)
7. behavior_insights (observation about communication style)
...
Generate the report as JSON:"""
        ...
        system = "You are a senior interviewer generating performance feedback. Be constructive and specific."
        
        try:
            response = await self._call_ollama(prompt, system)
            report = self._extract_json(response)
            
            if isinstance(report, dict) and "technical_score" in report:
                return report
        except Exception as e:
            print(f"[DEBUG] Report generation error: {e}")
        
        # Fallback report
        avg_score = sum(q.get('score', 0) for q in questions) // max(len(questions), 1)
        return {
            ...
            "improvements": [f"Practice more {skill_type} concepts", "Work on structuring answers"],
            "retry_recommendations": [f"Review {skill_type} fundamentals"],
            "confidence_insights": "Maintained composure throughout the interview.",
            "behavior_insights": "Clear communication style observed."
        }
